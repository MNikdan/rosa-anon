...
def build_composer_peft_model(
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map='cpu' if is_fsdp else 'auto',
...
def main(cfg: DictConfig):
    ...
    if rosa_config is not None:
        assert fsdp_config is None and ds_config is None, 'fsdp and deepspeed are not supported with rosa'
    else:
        assert fsdp_config is None or ds_config is None, 'fsdp and deepspeed are cannot both be enabled'
    ...
    if rosa_config is not None:
        from peft.tuners.rosa import RosaModel, RosaScheduler
    ...
    assert scheduler_name == 'linear_decay_with_warmup'

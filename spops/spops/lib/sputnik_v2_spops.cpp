...
#include "./sputnik_v2/sputnik/cuda_utils.h"
#include <vector>


std::vector<torch::Tensor> sputnik_v2_sddmm_cuda_fp32_benchmark( // returns masked(mm(A, BT.T), M)
...
std::vector<torch::Tensor> sputnik_v2_sddmm_cuda_fp32( // returns masked(mm(A, BT.T), M)
	// torch::Tensor C_val, // the output values
	torch::Tensor M_row_offsets,
	torch::Tensor M_row_indices,
	torch::Tensor M_col_indices,
...
std::vector<torch::Tensor> sputnik_v2_sddmm_fp32_benchmark(
	// torch::Tensor C_val, // the output values
	torch::Tensor M_row_offsets,
	torch::Tensor M_row_indices,
	torch::Tensor M_col_indices,
  ...
  return sputnik_v2_sddmm_cuda_fp32_benchmark(M_row_offsets, M_row_indices, M_col_indices, last0, last1, A, BT, csr, t);
}
...
std::vector<torch::Tensor> sputnik_v2_sddmm_fp32(
	// torch::Tensor C_val, // the output values
	torch::Tensor M_row_offsets,
	torch::Tensor M_row_indices,
	torch::Tensor M_col_indices,
...
// const at::cuda::OptionalCUDAGuard device_guard1(device_of(C_val));
  ...
  const at::cuda::OptionalCUDAGuard device_guard5(device_of(A));
  const at::cuda::OptionalCUDAGuard device_guard6(device_of(BT));
  return sputnik_v2_sddmm_cuda_fp32(M_row_offsets, M_row_indices, M_col_indices, last0, last1, A, BT, true);
}
